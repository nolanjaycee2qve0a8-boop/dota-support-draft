...
Run `python -m dota_support_draft.stratz_schema_probe` only after configuring `STRATZ_API_TOKEN` in the interactive environment. Phase 1 introspects relevant Query fields and prints argument types plus return types. Phase 2 follows a bounded number of relevant reachable named input/object types and prints a bounded field summary. It uses normal transport/error mapping, never prints the token, and neither persists nor dumps the schema response. A missing token prints `NOT RUN / TOKEN NOT CONFIGURED`. Its output must be reviewed before adding any production role/meta/pair query.
