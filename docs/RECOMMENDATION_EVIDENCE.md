...
The desktop UI shows the current semantic pair context locally: P4/P5 role, allied and enemy pick counts, and the deterministic ordered shortlist (at most eight heroes). It also distinguishes Counter and Synergy as available, pending, not requested, or unavailable without exposing raw component errors. With no related picks, or while a component is missing, the UI identifies the separately loaded Meta and Personal states rather than treating missing pair evidence as zero or making a generic availability promise. Search and candidate-table selection do not alter this context or start pair transport work.
...
DOTA-030 adds a local Pair action status that turns the existing state and per-component coverage into next-step guidance. It identifies updating, available, partial, unavailable, no-related-pick, and no-service states; independently loaded Meta and Personal remain visible in the adjacent base-evidence layer rather than being repeated as a generic promise. Its retry guidance refers only to the existing `Refresh pair evidence` action, which recalculates the current context through normal caches and does not force transport. The label itself schedules no work and does not expose raw failure details.
