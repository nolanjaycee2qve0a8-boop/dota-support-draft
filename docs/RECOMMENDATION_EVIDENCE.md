...
DOTA-030 adds a local Pair action status that turns the existing state and per-component coverage into next-step guidance. It identifies updating, available, partial, unavailable, no-related-pick, and no-service states; it repeats the independently loaded Meta and Personal states when pair evidence is absent. Its retry guidance refers only to the existing `Refresh pair evidence` action, which recalculates the current context through normal caches and does not force transport. The label itself schedules no work and does not expose raw failure details.
