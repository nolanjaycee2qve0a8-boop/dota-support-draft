...
- DraftState input adapters remain research-gated: DOTA-028 records no approved Dota GSI or third-party-file schema, no runtime collector, and no automatic draft replacement. Any future source must pass source provenance, local mapping, human-confirmation, atomic-replacement, and Qt lifecycle gates first.
- DOTA-029 adds the versioned `MANUAL_IMPORT` document contract, redacted fixtures, and test-local validation. DOTA-033 implements only its approved user-pasted JSON preview/explicit-confirmation path: no file I/O, watcher, GSI, clipboard reading, process/screen access, HTTP listener, or automatic replacement. A confirmed v1 import atomically replaces picks/bans/P4-P5 and clears manual ally position/lane assignments because v1 does not carry them.
