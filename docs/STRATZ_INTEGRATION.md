...
Run `python -m dota_support_draft.stratz_schema_probe` only after configuring `STRATZ_API_TOKEN` in the interactive environment. Phase 1 introspects relevant Query fields (including `constants`) and prints argument types plus return types with six fixed `ofType` wrapper levels. Phase 2 uses a bounded breadth-first traversal of reachable relevant named input/object types: at most 12 type probes, depth 3, and 14 fields per type; duplicate names are queried once. It uses normal transport/error mapping, never prints the token, and neither persists nor dumps the schema response. A missing token prints `NOT RUN / TOKEN NOT CONFIGURED`. Its output must be reviewed before adding any production role/meta/pair query.
